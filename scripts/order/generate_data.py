...
class GroupingGenerator(DataGenerator):
    ...
    def __init__(self, dataset_dir, config):
        super().__init__(dataset_dir, config, evals_allowed_in_train=4)
    ...
    def create_complete_facts(self, relation):
        ...
        for k in range(datagen_config.FACTS_PER_RELATION):
            ...
            # entities = list(range(100*(r_numb % 100)+(k+1)*6,
            #                       100*(r_numb % 100) + k*6, -1))
...
if __name__ == '__main__':
    ...
    shutil.copy(config.order_config, os.path.join(DATA_DIR, 'datagen_config.py'))