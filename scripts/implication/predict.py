...
if __name__ == '__main__':
    model = BertForMaskedLM.from_pretrained(
        os.path.join(config.output_dir, 'models', 'implication', '20_rules', 'checkpoint_86100'))
    tokenizer = BertTokenizer.from_pretrained(os.path.join(config.vocab_dirs['implication'],
                                                           '20_rules'))
    # masked_file = os.path.join(config.datasets_dirs['implication'], '20_rules', 'masked_eval.txt')
    # correct_json_file = os.path.join(config.datasets_dirs['implication'], 'StandardInv_with_anti',
    #                                  'subject_relation2object_eval.json')
    # predict(model, tokenizer, masked_file, correct_json_file)
    ids = tokenizer.convert_tokens_to_ids(['[CLS]', 'e0'])
    print(ids)
