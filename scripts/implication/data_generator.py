...
class ImplicationDataGenerator(ABC):
    ...
    def __init__(self, dataset_dir, config, evals_allowed_in_train, complete_pattern_per_line=False,
        ...
        self.attributes = ['a' + str(i) for i in range(self.conf.ATTRIBUTES_AMOUNT)]
    ...
    def create_dataset(self):
        ...
        for _ in range(self.conf.NUMBER_RULES):
            relations = sample(list(self.pattern_relations), 2)
            for _ in range(self.conf.IMPLICATIONS_PER_RELATION):
                cause = sample(self.attributes, 1)[0]
                effects = sample(self.attributes, 5)
                complete_facts = self.create_complete_facts(relations, cause, effects)
    ...
    def create_vocab(self):
        vocab = ["[SEP]", "[CLS]", "[PAD]", "[MASK]", "[UNK]"] + self.relations + self.entities + self.attributes
    ...
    @abstractmethod
    def create_complete_facts(self, relation, cause, effects):
        pass

    def split(self, facts, split_pos):
        train, eval = numpy.split(facts, [split_pos])
        structured_train = train
        ...
        if self.complete_pattern_per_line:
            ...
            # self.write_tsv(structured_train, to_be_masked)
    ...
    def write_tsv(self, train, to_be_masked):
        cause = train[0][0][2]
        available_samples = self.effects.copy()
        for effect in train[0][1:]:
            available_samples.remove(effect[2])
        for fact in train:
            self.classification_file.write(f'{fact[0][0]} {fact[0][1]} {cause}\t1\ttrain\n')
            for effect in fact[1:]:
                effect = ' '.join(effect)
                self.classification_file.write(f'{effect}\t1\ttrain\n')
            false_effects = sample(available_samples, 6)
            for false in false_effects:
                self.classification_file.write(f'{fact[0][0]} {fact[0][1]} {false}\t0\ttrain\n')

        for fact in to_be_masked:
            cause_fact = f'{fact[0][0]} {fact[0][1]} {cause}'
            self.classification_file.write(f'{cause_fact}\t1\ttrain\n')
            for effect in fact:
                effect_fact = ' '.join(effect)
                self.classification_file.write(f'{effect_fact}\t1\teval\n')
            false_effects = sample(available_samples, 6)
            for false in false_effects:
                self.classification_file.write(f'{fact[0][0]} {fact[0][1]} {false}\t1\teval\n')
