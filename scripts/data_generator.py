    ...
    def split(self, facts, split_pos):
        ...
        if self.complete_pattern_per_line:
            ...
            for chain in eval:
                ...
                to_be_masked += chain[-self.leave_out:]
            to_be_masked = list(filter(lambda x: self.check_train(x, train, 0), to_be_masked))
            if self.leave_out > 1:
                to_be_masked = numpy.array(to_be_masked)
                to_be_masked = numpy.array(to_be_masked).reshape(to_be_masked.shape[0]*to_be_masked.shape[1], to_be_masked.shape[2])
    ...
    def write(self, triples, type, obj_dict):
        ...
        with open(os.path.join(self.dir, f'{type if type != "rand_train" else type.replace("rand_", "")}.txt'), 'a') as file, \
            ...
            for triple in triples:
                ...
                obj_dict[self.build_string(triple, mask=True).replace(' [MASK]\n', '')].append(triple[-1][-1] if self.complete_pattern_per_line else triple[-1])
