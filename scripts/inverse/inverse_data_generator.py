...
class InverseDataGenerator(ABC):
    ...
    def create_dataset(self):
        ...
        for _ in range(self.conf.NUMBER_RULES):
            ...
            complete_facts, confuse = self.create_complete_facts(relations)
            ...
            self.write(confuse, 'anti_train', self.anti_subj_rel2obj_train)
        for _ in range(self.conf.NUMBER_RULES):
            relations = sample(list(self.random_relations), 1)[0]
            rand_train, rand_eval = self.create_incomplete_patterns(relations)
