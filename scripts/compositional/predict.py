...
model = BertForMaskedLM.from_pretrained(
    os.path.join(config.output_dir, 'models', 'compositional', 'StandardComp_with_anti','checkpoint-61600'))
model.eval()

tokenizer = BertTokenizer.from_pretrained(os.path.join(config.vocab_dirs[relation], 'StandardComp_with_anti'))

masked_file = os.path.join(config.datasets_dirs[relation], 'StandardComp_with_anti', 'masked_eval.txt')
correct_json_file = os.path.join(config.datasets_dirs[relation], 'StandardComp_with_anti',
